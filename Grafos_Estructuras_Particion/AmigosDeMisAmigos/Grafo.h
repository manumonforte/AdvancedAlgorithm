...
public://coste de la solucion O(V+E) 
	MaxComponenteConexa(Grafo const & gf) : marked(gf.V(), false),
		max(0) {
...
private:
	...
	int max;
	void maxConexa(Grafo const & gf, int v, int &tam){
		marked[v] = true;
		tam++;
		for (int y : gf.ady(v)){
			if (!marked[y]) maxConexa(gf, y, tam);
		}
	}
