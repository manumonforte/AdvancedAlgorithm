...
public:
	...
	void rango(std::vector<int> &sol, int k1, int k2){
		buscaRango(raiz, sol, k1, k2);
	}
...
protected:
	...
	void buscaRango(Link d, std::vector<Clave> &sol, Clave k1, Clave k2){//coste de la funcion O(N) , donde n es el numero de nodos
		if (d == nullptr) return; // si es vacio salimos
		if (menor(k1, d->cv.clave)) buscaRango(d->iz, sol, k1, k2);// si es mayor que k1 vamos hacia la izq
		if (!menor(d->cv.clave, k1) && !menor(k2, d->cv.clave)) sol.push_back(d->cv.clave); // si esta en el rango introducimos en vector
		if (menor(d->cv.clave, k2)) buscaRango(d->dr, sol, k1, k2);// si es menor que k2 vamos hacia la derecha
	}
