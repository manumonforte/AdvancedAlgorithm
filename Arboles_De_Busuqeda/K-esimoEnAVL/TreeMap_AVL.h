...
//-> Comparar tam_i sin utilizar objeto funcion menor, ya que es para claves
//-> lanzar excepcion si la pos esta fuera
//-> Coste de la funcion logN, n--> numero de nodos del arbol
...
protected:
	...
	int buscaK(Link d, int k){
		if (k > nelems) throw std::domain_error("ERROR");
		else{
			if (d == nullptr) throw std::domain_error("ERROR");
			if (d->iz == nullptr && d->dr == nullptr) return d->cv.clave;
			else{
				if (k < d->tam_i) return  buscaK(d->iz, k);
				else if (d->tam_i < k){
					k -= d->tam_i;
					return buscaK(d->dr, k);
				}
				else return d->cv.clave;
			}
		}
	}
	...
	static void rotaDer(Link & k2) {
		Link k1 = k2->iz;
		//obtenemos Y , ya que K1 mantiene tam_i
		int y;
		if (k2->iz == nullptr) y = 1;
		else y = k2->tam_i - k1->tam_i;
		k2->tam_i = y;
	...
	static void rotaIzq(Link & k1) {
		...
		int x, y;
		//obtenemos x e y
		if (k1->iz == nullptr) x = 0;
		else x = k1->tam_i - 1;
		if (k2->iz == nullptr) y = 0;
		else y = k2->tam_i - 1;

		// corregimos tam_i
		if (x && y)
			k2->tam_i = x + y + 1 + 1; // padre(x,y) + 1
		else if (!x && !y) {
			k2->tam_i = 1 + 1;
		}
		else if (!x) {
